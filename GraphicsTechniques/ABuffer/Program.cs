...
var opaqueWGSL = ResourceUtils.GetEmbeddedResource("ABuffer.shaders.opaque.wgsl", typeof(Program).Assembly);
...
return Run("A-Buffer", WIDTH, HEIGHT, async runContext =>
{
    ...
    {
        Size = teapotMesh.Positions.GetByteLength(),
        Usage = BufferUsage.Vertex,
        MappedAtCreation = true,
        Label = "vertexBuffer",
    });
    vertexBuffer.GetMappedRange<Vector3>(mapping =>
    {
        teapotMesh.Positions.AsSpan().CopyTo(mapping);
    });
    vertexBuffer.Unmap();
    ...
    indexBuffer.GetMappedRange<InlineArray3<ushort>>(mapping =>
    {
        for (int i = 0; i < teapotMesh.Triangles.Length; i++)
        {
            ref var span = ref mapping[i];
            ref var triangle = ref teapotMesh.Triangles[i];
            span[0] = (ushort)triangle.X;
            span[1] = (ushort)triangle.Y;
            span[2] = (ushort)triangle.Z;
        }
    });
    indexBuffer.Unmap();
    ...
    var opaqueModule = device.CreateShaderModuleWGSL("opaqueModule", new()
    {
        Code = opaqueWGSL
    });
    ...
    var opaquePassDescriptor = new RenderPassDescriptor()
    {
        ColorAttachments = [
            new()
            {
                View = null, // set later
                ClearValue = new Color(0, 0, 0, 1.0f),
                LoadOp = LoadOp.Clear,
...
class Settings
{
    public MemoryStrategyType MemoryStrategy { get; set; } = MemoryStrategyType.Multipass;
}
...
struct Uniforms
{
    ...
    public uint MaxStorableFragments;
    public uint TargetWidth;
    private InlineArray2<float> _padding;
}
