...
static double GetNumSteps(int numElements)
{
    var n = Math.Log2(numElements);
    return n * (n + 1) / 2;
}

var atomicToZeroWGSL = ResourceUtils.GetEmbeddedResource("BitonicSort.shaders.atomicToZero.wgsl");

CommandBuffer DrawGui(GuiContext guiContext, Surface surface, Action drawGuiContents)
{
    guiContext.NewFrame();
    ImGui.SetNextWindowBgAlpha(0.75f);
    ImGui.SetNextWindowPos(new(0, 0));
    ImGui.SetNextWindowSize(new(350, 240));
    ImGui.Begin("Bitonic Sort",
...
return Run("Bitonic Sort", WIDTH, HEIGHT, async runContext =>
{
    ...
    {
        RequiredFeatures = hasTimestampQuery ? [FeatureName.TimestampQuery] : null,
    ...
    var maxInvocationsX = deviceLimits.MaxComputeWorkgroupSizeX;

    QuerySet? querySet = null;
    GPUBuffer? timestampQueryResolveBuffer = null;
    GPUBuffer? timestampQueryResultBuffer = null;
    if (hasTimestampQuery)
    {
        ...
        {
            // 2 timestamps * long size for nanoseconds
            Size = 2 * (ulong)sizeof(long),
    ...
    var settings = new Settings
    {
        ...
        WorkgroupsPerStep = (int)(maxElements / ((ulong)maxInvocationsX * 2)),
        HoveredCell = 0,
        SwappedCell = 1,
        StepIndex = 0,
    ...
    var elementsBufferSize = (ulong)(sizeof(float) * totalElementOptions[0]);
    ...
    var computeBGClusterBindGroupLayout = device.CreateBindGroupLayout(new()
    {
        Label = "BitonicSort.bindGroupLayout",
        Entries = [
            new()
            {
                Binding = 0,
    ...
    var computeBGClusterBindGroup = device.CreateBindGroup(new()
    {
        Label = "BitonicSort.bindGroup",
        Layout = computeBGClusterBindGroupLayout,
        Entries = [
            new()
            {
                Binding = 0,
    ...
    var computePipeline = device.CreateComputePipeline(new()
    {
        ...
        {
            BindGroupLayouts = [computeBGClusterBindGroupLayout]
        }),
        Compute = new()
        {
    ...
    var renderPassDescriptor = new MangedRenderPassDescriptor
    {
        ColorAttachments = [
            new()
            {
                ...
                ClearValue = new Color(0.1f, 0.4f, 0.5f, 1.0f),
                LoadOp = LoadOp.Clear,
    ...
    void ResetTimeInfo()
    {
        ...
        var nanCheck = settings.ConfigToCompleteSwapsMap[settings.ConfigKey].Time /
            settings.ConfigToCompleteSwapsMap[settings.ConfigKey].Sorts;
        var ast = double.IsNaN(nanCheck) ? 0 : nanCheck;
        settings.AverageSortTimeString = $"{ast:F5}ms";
    }

    void ResetExecutionInformation()
    {
        settings.WorkgroupSize = (uint)Math.Min(settings.SizeLimit, maxInvocationsX);
        settings.WorkgroupsPerStep = settings.TotalElements / ((int)settings.WorkgroupSize * 2);
        ...
        var newCellWidth = (int)(Math.Sqrt(settings.TotalElements) % 2 == 0
            ? Math.Floor(Math.Sqrt(settings.TotalElements))
            : Math.Floor(Math.Sqrt(settings.TotalElements / 2)));

        var newCellHeight = settings.TotalElements / newCellWidth;
        settings.GridWidth = newCellWidth;
        settings.GridHeight = newCellHeight;
        settings.GridDimensions = $"{settings.GridWidth}x{settings.GridHeight}";

        // Set prevStep to None (restart) and next step to FLIP
        settings.PrevStep = Step.None;
        settings.NextStep = Step.FlipLocal;
        ...
        computePassEncoder.SetBindGroup(0, computeBGClusterBindGroup);
    ...
    void RandomizeElementArray()
    {
        ...
        while (currentIndex != 0)
        {
            ...
            var randomIndex = (int)(Random.Shared.NextSingle() * currentIndex);
            currentIndex--;
    ...
    void endSortInterval(object state)
    {
        autoSortTimer?.Dispose();
        autoSortTimer = null;
    }

    void startSortInterval()
    {
        var currentIntervalSpeed = settings.AutoSortSpeed;
        autoSortTimer = new Timer(s =>
        {
            if (settings.NextStep == Step.None)
            {
                autoSortTimer?.Dispose();
                autoSortTimer = null;
            }
            if (settings.AutoSortSpeed != currentIntervalSpeed)
            {
                autoSortTimer?.Dispose();
                autoSortTimer = null;
                startSortInterval();
            ...
            SetSwappedCell();
        }, null, dueTime: 0, period: settings.AutoSortSpeed);
    }


    void DrawComputeResourcesFolder()
    {
        if(ImGui.CollapsingHeader("Compute Resources", ImGuiTreeNodeFlags.DefaultOpen))
        {
            ImGui.Combo("Total Elements", ref settings.TotalElements, totalElementOptions, totalElementOptions.Count);
        }
    }
...
enum Step
{
    // No sort step has or will occur
    None,
    // A sort step that performs a flip operation over indices in a workgroup's locally addressable area
    // (i.e invocations * workgroup_index -> invocations * (workgroup_index + 1) - 1.
    FlipLocal,
    //A sort step that performs a flip operation over indices in a workgroup's locally addressable area.
    DisperseLocal,
    //A sort step that performs a flip step across a range of indices outside a workgroup's locally addressable area.
    FlipGlobal,
    //A sort step that performs a disperse operation across a range of indices outside a workgroup's locally addressable area.
    DisperseGlobal
}
...
class Settings
{
    ...
    public Dictionary<string, ConfigInfo> ConfigToCompleteSwapsMap;
